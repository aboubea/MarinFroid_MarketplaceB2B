...
4. Configurer les variables d'environnement dans Vercel : `DATABASE_URL`, `AUTH_SECRET`, `RESEND_API_KEY`, `RESEND_FROM`, `APP_URL` (domaine de production : `https://www.e-marinfroid.fr`, sans slash final — déclaré aussi dans **Settings → Domains**, avec les enregistrements DNS correspondants chez le registrar), et optionnellement `CRON_SECRET` (protège `/api/cron/deliveries`, appelé deux fois par jour — voir `apps/web/vercel.json` → `crons` — pour faire passer automatiquement les commandes du jour de "en préparation" à "expédiée" le matin, puis à "livrée" en soirée ; les statuts de commande ne se changent jamais manuellement côté admin).
