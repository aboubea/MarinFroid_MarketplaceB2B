 ...
 * for anti-aliasing) so the mark reads correctly wherever it's displayed
 * (dark sidebar, white form, photo backdrop...). SVGs are rasterized too,
 * since they commonly bake in a background rect that vector output alone
 * wouldn't strip.
 */
function stripSolidBackground(file: File): Promise<File> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // SVGs report a tiny intrinsic size (often 300x150) unless explicit
      // width/height are set — upscale so the strip mask stays crisp.
